...
class SegmentationConfusions(object):
    def collect(true, pred, **kwargs):
    ...
    def allowed_average_types():
        return { 'micro', 'macro', 'none' }

class PoundDetectionConfusions(object):
    ...
    def collect(true, pred, **kwargs):
        ...
        points = PoundDetectionConfusions._get_central_points(true)
        pred_points = PoundDetectionConfusions._get_central_points(pred)

        confusions = defaultdict(dict)
        confusions[1] = PoundDetectionConfusions._compute_confusion(points, pred_points, kwargs['distance'])
